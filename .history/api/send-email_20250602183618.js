...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Option 4: Simple fallback using a basic email service (Formspree alternative)
    try {
      // This uses a simple HTTP POST to a email forwarding service
      // You can replace this with any email API service you prefer
      const response = await fetch('https://formsubmit.co/ajax/leads-aaaaayhbcsc2dawosfuuiidvm4@sixtysecondsapp.slack.com', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (response.ok) {
        res.status(200).json({ message: 'Email sent successfully via FormSubmit' });
        return;
      }
    } catch (error) {
    ...
    // If no email service worked, return error but don't fail completely
    res.status(500).json({ 
      error: 'Email service temporarily unavailable', 
      message: 'Please try again or contact us directly at andrew.bryce@sixtyseconds.video',
      fallback: true
    });
