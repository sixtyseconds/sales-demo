...
Timestamp: ${new Date().toLocaleString()}`;
      ...
      // Method 1: Try to send via fetch to a simple endpoint (you can set this up later)
      try {
        const response = await fetch('/api/send-email', {
          ...
          body: JSON.stringify({
            to: ['app@sixtyseconds.video', 'andrew.bryce@sixtyseconds.video'],
        ...
        console.log('API endpoint not available, falling back to mailto');
      }

      // Method 2: Fallback to mailto link
      const mailtoLink = `mailto:app@sixtyseconds.video,andrew.bryce@sixtyseconds.video?subject=${encodeURIComponent(emailSubject)}&body=${encodeURIComponent(emailBody)}`;
